...
x_analog2 = sum((ones(length(t),1)*x).*triangular_array,2); %Triangular
Reconstruction
x_analog3 = sum((ones(length(t),1)*x).*rec_array,2); % Rectangular
Reconstruction
% Residual Signals
r1=x_cont-x_analog1;
r2=x_cont-x_analog2;
...
figure;
plot(t(1:1000),x_cont(1:1000),'b--','LineWidth',2) % Plot original analog
signal
hold on
plot(n(1:dt/Ts*1000)*Ts,x(1:dt/Ts*1000),'bx','MarkerSize',14) % Plot Sample
Points
